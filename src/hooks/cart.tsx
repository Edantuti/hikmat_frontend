...
function totalDiscount(data: any) {
  return data.map((current: any) => {
    return (
      current.discount +
      current.Deals.reduce((sum: number, curr: any) => {
...
function totalPrice(data: any) {
  return data.reduce(
    (sum: number, curr: any) => sum + curr.cart_quantity * curr.price,
    0
  );
}
...
const useFetchCart = (userid: string) => {
  ...
  React.useEffect(() => {
    axios
      .get(`${import.meta.env.VITE_BACKEND}/api/cart`, {
        params: {
          userid: userid,
        },
        headers: {
...
export { useFetchCart, totalDiscount, totalPrice };
